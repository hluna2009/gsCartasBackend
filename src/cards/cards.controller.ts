...
import { ReopenCardDto } from './dto/reopen-card.dto';
...
export class CardsController {
  ...
  @Get('report')
  @ApiOperation({ summary: 'Obtener todas las Cartas Report' })
  ...
  findAllReport(@Query() paginationDto: PaginationDto) {
    return this.cardsService.findAllReport();
  }
  ...
  async assignedCard(
    @Param('id') id: string,
    @Body() assignedCardDto: AssignedCardDto,
  ) {
  ...
  async closeCard(
    @Param('id') id: string,
    @Body() pendingCardDto: PendingCardDto,
  ) {
  ...
  @ApiResponse({ status: 200, description: 'Carta actualizada exitosamente' })
  @ApiResponse({ status: 404, description: 'Carta no encontrada' })
  async reopenCard(@Param('id') id: string, @Body() body: ReopenCardDto) {
    return this.cardsService.reopenCard(+id, body);
  }
  ...
  @ApiResponse({ status: 200, description: 'Carta actualizada exitosamente' })
  @ApiResponse({ status: 404, description: 'Carta no encontrada' })
  async assignmentCard(
    @Param('id') id: string,
    @Body() assignmentCardDto: AssignmentCardDto,
  ) {
